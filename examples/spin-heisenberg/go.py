...
n_steps = 500000  # Number of steps / 步数

# Model parameters / 模型参数
J = 0.001  # Exchange coupling in eV (positive = ferromagnetic) / 交换耦合（eV）
...
box = cp.array([[L * a, 0, 0], [0, L * a, 0], [0, 0, L * a]], dtype=cp.float64)
...
nb = CudaNeighborList(N, 10, 2.6)
force_model = HeisenbergForceModel(J=J, cutoff=cutoff, neighbor_list=nb)
...
def get_spin_temperature(sim):
    ...
    return float(sim.state.spin.get_spin_temperature(
...
atoms_dump = AtomsDump("trajectory.xyz", interval=2000)
